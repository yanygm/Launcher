...
namespace KartRider.IO.Packet
{
	public class InPacket : PacketBase
	{
		...
		public string ReadNullTerminatedString(bool ascii = false)
		{
			StringBuilder stringBuilder = new StringBuilder();
			if (ascii)
			{
				ushort num;
				while ((num = (ushort)this.ReadByte()) > (ushort)0)
					stringBuilder.Append((char)num);
			}
			else
			{
				ushort num;
				while ((num = this.ReadUShort()) > (ushort)0)
					stringBuilder.Append(Encoding.Unicode.GetString(BitConverter.GetBytes(num)));
			}
